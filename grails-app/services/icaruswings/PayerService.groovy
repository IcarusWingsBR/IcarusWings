...
class PayerService {
    ...
    private Payer validateSave(PayerAdapter payerAdapter) {
        ...
        } else if (!StringUtils.dontHaveNumber(payerAdapter.name) || payerAdapter.name.length() < 3 || payerAdapter.name.length() > 255) {
        ...
        } else if (!StringUtils.dontHaveNumber(payerAdapter.city) || payerAdapter.city.length() < 3 || payerAdapter.city.length() > 50) {
        ...
        } else if (!StringUtils.dontHaveNumber(payerAdapter.state) || payerAdapter.state.length() < 2 || payerAdapter.state.length() > 15) {
